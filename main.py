...
from model.slu_bert_tagging import SLUBertTagging
from model.slu_baseline_tagging import SLUTagging
from model.slu_MISCASlot_Attention_tagging import  MISCA_Att
from model.slu_MISCASlot_noAttention_tagging import  MISCA_noAtt
...
elif args.model_name == 'MISCA_att':
    torch.cuda.empty_cache()
    model = MISCA_Att(args).to(device)
    
elif args.model_name == 'MISCA_noAtt':
    torch.cuda.empty_cache()
    model = MISCA_noAtt(args).to(device)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        print('Training: \tEpoch: %d\tTime: %.4f\tTraining Loss: %.4f\tTraining acc: %.4f' % (
        i, time.time() - start_time, epoch_loss / count, train_acc))
        torch.cuda.empty_cache()
        gc.collect()
