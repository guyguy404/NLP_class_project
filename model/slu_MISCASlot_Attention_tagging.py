...
class MISCADecoder(nn.Module):
    ...
    def forward(self, encoded, mask, labels=None,seq_lens=20):
        ...
        i_context_vector, slot_logits, i_attn = perform_attention(self, 'slot', slot_output, seq_lens)
        reshaped_logits = []
        for logit in slot_logits:
            reshaped_logits.append(logit.reshape(int(logit.shape[0]),1,int(logit.shape[1])))
            
            
        slot_logits = torch.cat(reshaped_logits, dim=1)
